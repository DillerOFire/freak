...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if should_reply(update.message, f"@{bot_username}"):
        ...
        # 3. Call LLM
        response = await generate_response(list(chat_history), user_thoughts, general_memories)
        ...
        if response and response.get("content"):
            ...
            try:
                if reply_to:
                    await context.bot.send_message(chat_id=update.effective_chat.id, text=content, reply_to_message_id=reply_to)
                else:
