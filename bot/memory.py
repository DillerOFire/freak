...
async def get_user_thought(user_id: int) -> str:
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT thoughts FROM users WHERE user_id = ?", (user_id,)) as cursor:
...
async def get_general_memories(limit: int = 5) -> list[str]:
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT topic, summary FROM general_memory ORDER BY timestamp DESC LIMIT ?", (limit,)) as cursor:
            rows = await cursor.fetchall()
            return [f"Topic: {row[0]}, Summary: {row[1]}" for row in rows]

async def add_general_memory(topic: str, summary: str):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("INSERT INTO general_memory (topic, summary) VALUES (?, ?)", (topic, summary))
        await db.commit()
