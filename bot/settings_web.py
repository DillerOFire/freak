...
<head>
  ...
  <style>
    :root { color-scheme: light dark; --bg: var(--tg-theme-bg-color, #10131a); --card: var(--tg-theme-secondary-bg-color, #1a202b); --text: var(--tg-theme-text-color, #edf1f7); --hint: var(--tg-theme-hint-color, #98a2b3); --accent: var(--tg-theme-button-color, #61a8ff); --accent-text: var(--tg-theme-button-text-color, #fff); --bad: #f97066; --good: #4bb57b; }
    * { box-sizing: border-box; }
    ...
    textarea { min-height: 220px; resize: vertical; } input[type=number] { max-width: 180px; }
    ...
    button { width: 100%; border: 0; border-radius: 10px; padding: 13px; background: var(--accent); color: var(--accent-text); font: 700 16px system-ui, sans-serif; cursor: pointer; }
...
<body>
  <main>
    ...
    <form id="settings-form">
      <section class="card"><h2>Environment</h2><div id="environment"></div></section>
      <section class="card"><h2>Persona</h2><p id="default-persona-note" class="hint hidden">This is the built-in default. Saving it makes it a custom prompt.</p><div class="field"><label for="persona">Persona prompt</label><textarea id="persona" maxlength="30000"></textarea></div></section>
      <section class="card"><h2>Global behavior</h2>
  ...
  <script>
    ...
    async function request(method, data) {
      const response = await fetch(api, { method, headers: { 'Content-Type': 'application/json', 'X-Telegram-Init-Data': telegram ? telegram.initData : '' }, body: data ? JSON.stringify(data) : undefined });
    ...
    function populate(data) {
      ...
      setField('persona', data.persona_prompt); document.getElementById('default-persona-note').classList.toggle('hidden', !data.persona_uses_default);
