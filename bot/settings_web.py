...
<body>
  ...
  (() => {
    ...
    function summaryItems(s, role) {
      const base = [['Events', fmt(s.total_events)], ['Success rate', rate(s.success_rate)], ['Failure rate', rate(s.failure_rate)], ['Avg latency', s.avg_latency_ms == null ? 'n/a' : `${fmt(s.avg_latency_ms)} ms`], ['Avg prompt tokens', fmt(s.avg_prompt_tokens)], ['Avg uncached tokens', fmt(s.avg_uncached_prompt_tokens)], ['Cached prompt tokens', fmt(s.avg_prompt_cached_tokens)], ['Weighted cached share', rate(s.weighted_prompt_cache_hit_rate)], ['Cache-hit calls', rate(s.prompt_cache_hit_call_rate)], ['Cache writes', fmt(s.avg_prompt_cache_write_tokens)]];
      if (role === 'main') {
